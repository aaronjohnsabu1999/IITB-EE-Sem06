/*
 * This file is part of oled library for ssd1306/ssd1309/sh1106 oled-display.
 *
 * oled library for ssd1306/ssd1309/sh1106 oled-display is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or any later version.
 *
 * oled library for ssd1306/ssd1309/sh1106 oled-display is distributed in the hope that it will be useful,
 ...
 *
 * Diese Datei ist Teil von oled library for ssd1306/ssd1309/sh1106 oled-display.
 *
 * oled library for ssd1306/ssd1309/sh1106 oled-display ist Freie Software: Sie können es unter den Bedingungen
 * der GNU General Public License, wie von der Free Software Foundation,
 * Version 3 der Lizenz oder jeder späteren
 * veröffentlichten Version, weiterverbreiten und/oder modifizieren.
 *
 * oled library for ssd1306/ssd1309/sh1106 oled-display wird in der Hoffnung, dass es nützlich sein wird, aber
...
#pragma mark OLED COMMUNICATION
...
void oled_init(uint8_t dispAttr){
    ...
    OLED_DDR |= (1 << CS_PIN)|(1 << DC_PIN)|(1 << RES_PIN);
    OLED_PORT |= (1 << CS_PIN)|(1 << DC_PIN)|(1 << RES_PIN);
    OLED_PORT &= ~(1 << RES_PIN);
    _delay_ms(10);
    OLED_PORT |= (1 << RES_PIN);
#endif
...
void oled_clrscr(void){
    ...
    for (uint8_t i = 0; i < DISPLAY_HEIGHT/8; i++){
        ...
        oled_gotoxy(0,i);
        oled_data(displayBuffer[i], sizeof(displayBuffer[i]));
    }
#elif defined TEXTMODE
    ...
    switch (c) {
        ...
        case '\t':
            ...
            if( (cursorPosition.x+charMode*4) < (DISPLAY_WIDTH/ sizeof(FONT[0])-charMode*4) ){
                oled_gotoxy(cursorPosition.x+charMode*4, cursorPosition.y);
            }else{
                oled_gotoxy(DISPLAY_WIDTH/ sizeof(FONT[0]), cursorPosition.y);
            }
            break;
        case '\n':
            // linefeed
            if(cursorPosition.y < (DISPLAY_HEIGHT/8-1)){
                oled_gotoxy(cursorPosition.x, cursorPosition.y+charMode);
            }
            break;
        case '\r':
            // carrige return
            oled_gotoxy(0, cursorPosition.y);
            break;
        default:
            ...
            if (charMode == DOUBLESIZE) {
                ...
                oled_data(data, sizeof(FONT[0])*2);
...
void oled_drawRect(uint8_t px1, uint8_t py1, uint8_t px2, uint8_t py2, uint8_t color){
    if( px1 > DISPLAY_WIDTH-1 ||
       px2 > DISPLAY_WIDTH-1 ||
       py1 > DISPLAY_HEIGHT-1 ||
       py2 > DISPLAY_HEIGHT-1) return;
    oled_drawLine(px1, py1, px2, py1, color);
    oled_drawLine(px2, py1, px2, py2, color);
    oled_drawLine(px2, py2, px1, py2, color);
    oled_drawLine(px1, py2, px1, py1, color);
}
void oled_fillRect(uint8_t px1, uint8_t py1, uint8_t px2, uint8_t py2, uint8_t color){
    if( px1 > px2){
        uint8_t temp = px1;
        px1 = px2;
        px2 = temp;
    ...
    for (uint8_t i=0; i<=(py2-py1); i++){
        oled_drawLine(px1, py1+i, px2, py1+i, color);
    }
}
void oled_drawCircle(uint8_t center_x, uint8_t center_y, uint8_t radius, uint8_t color){
    ...
    oled_drawPixel(center_x  , center_y+radius, color);
    oled_drawPixel(center_x  , center_y-radius, color);
    oled_drawPixel(center_x+radius, center_y  , color);
    oled_drawPixel(center_x-radius, center_y  , color);
    ...
    while (x<y) {
        ...
        oled_drawPixel(center_x + x, center_y + y, color);
        oled_drawPixel(center_x - x, center_y + y, color);
        oled_drawPixel(center_x + x, center_y - y, color);
        oled_drawPixel(center_x - x, center_y - y, color);
        oled_drawPixel(center_x + y, center_y + x, color);
        oled_drawPixel(center_x - y, center_y + x, color);
        oled_drawPixel(center_x + y, center_y - x, color);
        oled_drawPixel(center_x - y, center_y - x, color);
    }
}
void oled_fillCircle(uint8_t center_x, uint8_t center_y, uint8_t radius, uint8_t color) {
    for(uint8_t i=0; i<= radius;i++){
        oled_drawCircle(center_x, center_y, i, color);
    }
}
void oled_drawBitmap(uint8_t x, uint8_t y, const uint8_t *picture, uint8_t width, uint8_t height, uint8_t color){
...
#if defined (SSD1306) || defined (SSD1309)
    oled_gotoxy(0,0);
    oled_data(&displayBuffer[0][0], DISPLAY_WIDTH*DISPLAY_HEIGHT/8);
#elif defined SH1106
    for (uint8_t i = 0; i < DISPLAY_HEIGHT/8; i++){
        oled_gotoxy(0,i);
        oled_data(displayBuffer[i], sizeof(displayBuffer[i]));
...
void oled_display_block(uint8_t x, uint8_t line, uint8_t width) {
    ...
    oled_goto_xpix_y(x,line);
    oled_data(&displayBuffer[line][x], width);
}
#endif